...
Here, the \colkey{anchor point} coordinate (which has no drawing and no label) is placed halfway along the interaction arc.

Often in ER maps, one wants to draw horizontal or vertical modulation arcs departing from outcomes.
Hence, the anchor point should be placed at the X-coordinate (for a vertical arc) or Y-coordinate (for an horizontal arc) of the outcome.
This can be achieved using \texttikz{}'s \mintinline{latex}{let} syntax for arcs, that allows using the coordinates of other nodes or points.
Following is an example of how to draw a vertical modulation arc outgoing from an outcome:
